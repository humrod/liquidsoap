...
module Make (Config : Config_t) = struct
  ...
  class http ~kind ~protocol ~playlist_mode ~poll_delay ~track_on_meta
    ...
    object (self)
      ...
      (* Mutex to change the internal values *)
  ...
  let register protocol =
    Lang.add_operator ("input." ^ protocol) ~return_t:(Lang.univ_t ())
      ~method_t:
        [
          ("start", ([], Lang.fun_t [] Lang.unit_t));
          ("stop", ([], Lang.fun_t [] Lang.unit_t));
          ("url", ([], Lang.fun_t [] Lang.string_t));
          ( "set_url",
            ( [],
              Lang.fun_t [(false, "", Lang.fun_t [] Lang.string_t)] Lang.unit_t
            ) );
          ("status", ([], Lang.fun_t [] Lang.string_t));
          ("buffer_length", ([], Lang.fun_t [] Lang.float_t));
        ]
      ~meth:
        [
          ( "start",
          ...
          ( "stop",
            fun s ->
              Lang.val_fun [] (fun _ ->
                  s#stop_cmd;
                  Lang.unit) );
          ("url", fun s -> Lang.val_fun [] (fun _ -> Lang.string s#url_cmd));
          ( "set_url",
            fun s ->
